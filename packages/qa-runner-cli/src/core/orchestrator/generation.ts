...
export type GenerationInput<TTestType extends string> = {
  ...
  healingAttempts?: HealingAttemptInput[];
};
...
export type HealingAttemptInput = {
  testId?: string;
  recovered: boolean;
  strategy?: string;
  occurredAt?: string;
...
export const buildHealingSummary = (attempts: HealingAttemptInput[]): HealingSummary => {
  const summary: HealingSummary = {
    totalAttempts: attempts.length,
    recoveredAttempts: attempts.filter((attempt) => attempt.recovered).length,
    byStrategy: {},
  };

  for (const attempt of attempts) {
    const key = attempt.strategy ?? "unknown";
    if (!summary.byStrategy[key]) {
      summary.byStrategy[key] = { attempts: 0, recovered: 0 };
    }
    summary.byStrategy[key].attempts += 1;
    if (attempt.recovered) {
      summary.byStrategy[key].recovered += 1;
    }
  }
...
export async function runGeneration<TTestType extends string>(input: GenerationInput<TTestType>): Promise<GenerationResult<TTestType>> {
  ...
  const healingAttempts = input.healingAttempts ?? [];
  const manifest: QaRunnerManifest = {
    ...
    healing: {
      attempts: healingAttempts.map((attempt) => ({
        testId: attempt.testId,
        recovered: attempt.recovered,
        strategy: attempt.strategy,
        occurredAt: attempt.occurredAt ?? new Date().toISOString(),
      })),
      summary: buildHealingSummary(healingAttempts),
    },
  };
